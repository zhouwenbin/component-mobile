    ...
    var a520 = can.Control.extend({
      ...
      countdown: function(time) {
        ...
        setTimeout(function() {
          if (time > 0) {
            time--;
            that.element.find('#mobile-code-btn').text(time + '秒后可重新发送').addClass('btn-disable');
            that.countdown.call(that, time);
          } else {
            that.element.find('#mobile-code-btn').text('发送短信验证码').removeClass('btn-disable');
          }
        }, 1000);
      },
      ...
      '#inviteTaBtn click': function($element, event){
        ...
        var message = $(".messagedetail").not(".hide").text();
      ...
      '[name="radio"] click': function($element, event) {
        var name = $element.parents().attr("name");
        $(".messagedetail").addClass("hide");
        $(".messagedetail[name='" + name + "']").removeClass("hide");
      }

    });

    new a520(".sf-b2c-mall-520");
  });
