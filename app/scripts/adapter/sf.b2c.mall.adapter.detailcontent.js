...
define('sf.b2c.mall.adapter.detailcontent', ['can'], function(can) {
  return can.Map({
    ...
    formatItemInfo: function(detailContentInfo, itemInfoData) {
      ...
      var logisticsend = detailContentInfo.itemInfo.saleInfo.logisticsEnd;
      var logisticsstart = detailContentInfo.itemInfo.saleInfo.logisticsStart;
      if (parseInt(logisticsend, 10) > parseInt(logisticsstart, 10)) {
        detailContentInfo.itemInfo.saleInfo.sendTime = logisticsstart + '-' + logisticsend;
      } else {
        detailContentInfo.itemInfo.saleInfo.sendTime = logisticsstart;
      }
    ...
    setSelectedSpec: function(index, specId, group) {
      ...
      if (typeof selectedSpec.attr != 'undefined') {
        selectedSpec.attr("selected", "btn-danger");
        ...
        selectedSpec.selected = "btn-danger";
      ...
      _.each(group.specs, function(spec) {
        ...
        if (typeof result != 'undefined') {
          ...
          if (typeof showDottedLine != 'undefined') {
            if (typeof spec.attr != 'undefined') {
              spec.attr("canShowDottedLine", "btn-dashed");
              spec.attr("compose", showDottedLine.skuSpecTuple.specIds.join(","));
            } else {
              spec.canShowDottedLine = "btn-dashed";
